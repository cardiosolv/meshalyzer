...
\begin{description} \itemsep1pt \parskip0pt \parsep0pt
  ...
  \item[FLTK 1.3] This is the widget set responsible for all the windows. It is recommended to 
    download this from \texttt{www.fltk.org} and compile it yourself.
  \item[GLUT] This is needed for offscreen rendering on a graphics card.
  \item[OSMesa] Required for offscreen rendering in software.
...
For surface elements, a region label string may also be specified. It need only be defined
...
\begin{description}
    ...
    \item[--compSurf=\emph{[[+]file]}] compute and output boundary surfaces and exit. If not
        specified, use model name. If ``+'' is not specified, exit after writing file. 
        To not write a file, specify file as ``/dev/null''.
\end{description}
...
surfaces and regions, and click exactly on top of it.
In the event of nearby vertices, the one closest to the viewer's eye (minimum z
window coordinate) will be selected.
Note that the value of the highlighted node can be changed by using the mouse wheel (ctrl and alt increase the 
rate of change) or by holding mouse buttons and dragging (the particular button determines the rate of change).
...
Pressing mouse button 3 pops up a small menu to reverse the colour scale, or make the colour scale symmetric about 0 based 
on either the maximum or minimum data value.
...
The initial starting point for the stride, \emph{start}, can also be set.
...
\label{sec:surface}
Only surfaces checked in the list are affected by operations.
...
Surfaces are renamed with the ``Rename'' button. You will be prompted if you
...
If checked, do not bother with elements which are facing away from the screen. 
Normally, they are eliminated because of depth checking. 
This should result in faster rendering of large meshes as well as brighter images when transparency is used.
However, with nonclosed surfaces, results may not be correct.
...
During off rotating with the trackball. This is automatically set for 1D and
2D data.
...
EPS and PDF are not recommended.

Note also that PNG files have the colour scale range  recorded in the \emph{Description}  field 
of the metadata,
...
Key & Action \\ \hline
    $\leftarrow$ & decrement time \\
    $\rightarrow$ & increment time \\
    c & pop control window \\
    o & recalibrate the colour scale to the optimal for the displayed data \\
    p  & enter \hyperref[para:pickvertex]{vertex picking} mode\\
    r  & reread the current data file \\
    t & \hyperref[sec:link]{temporally sync} \\
    v & \hyperref[sec:link]{viewport sync} \\
    \hline
\end{longtable}
...
At the top is the Process ID of the current \meshal instance below which is a list of the
currently running but unlinked \meshal instances.
To link, select a process and press the ``link selected'' button.
Linked processes are shown in the bottom list.
...
all processes. 
Unlinking is also bidirectional so unlinking it removes it entirely from the group.
...
\item[Temporal syncing] Clicking on the ``sync'' button (beside the animation pause button)
      ...
      model window.
\item[Viewport syncing] Clicking ``Image/Sync/Viewport'' or pressing ``v'' in the model window 
     will transmit the current scale, relative orientation, and relative translation to all other instances.
     If the meshes are not oriented along the same axes, a new reference direction can be defined and then the rotation 
     relative to this orientation will be applied. 
     The simplest way to use this is to 
     set the linked meshes 
     to their respective default views, orient one a mesh to match the others, 
     and then click ``Image/Sync/Set reference view''.
     Clicking ``Image/Sync/Reference view'' will put the mesh in the reference view.
 \item[Colour Scale] Clicking ``Image/Sync/Colour Scale'', or pressing the sync button 
     beside the colour bar will transmit the current colour map to all other instances.
 \item[Clipping planes]  Clicking ``Image/Sync/Clip'' or pressing the button on the clipping plane dialogue widget
     will send the clipping plane info to the linked instances. As with viewport suncing, the clipping is performed 
...
\section{FlyBy's}
\label{sec:flyby}
FlyBy's are animated views of the model. They are defined as occurring between two different model states. As implemented in \meshal, the 
model is put into the start start and then the view is transformed in a sequence of steps,
using \emph{Slerp}, until it matches the view corresponding to the second state, at which point the second state is loaded.
FlyBy's are made with the dialogue \texttt{Options/Fly By}.

\begin{figure}[H]
       \centerline{ \includegraphics[width=5cm]{flyby.png}}
       \caption{FlyBy dialogue widget. The play and record buttons (bottom row of buttons)
       only activate when two or more states are in the list.}
\end{figure}

To make a FlyBy, at least two states need to be defined.
To define a state, adjust the model to get the desired view, data, lighting, etc. and then press the \texttt{+} button to add it to the list.
As many states as desired can be added.
States can be deleted from the list with the \texttt{Delete} button and reordered with the Up/Down arrow buttons. 
To see a particular state, select it in the list, and press the load (circular arrow) button.
States can also be renamed to be more informative.

To view the FlyBy, press the play button. The time between frames is controlled by the \texttt{delay} value in milliseconds.
If data is present, the \texttt{adv time} button can be checked so that the time advances with every view change.
To record the FlyBy, set the output base name and click on the record (red dot) button.
A series of PNG files will be produced with the base name and then the frame number (00000,00001, $\ldots$) appended.

Remember that the  \texttt{\#steps} refers to the number of transition to go from the selected state to the next one.
...
The simplest way to use offscreen rendering is to interactively 
set up meshalyzer in a suitable state and then save it. The model,
data and state files are then specified normally on the command line
and the additional PNG related options added.
...
slower but can be compensated for by specifying many processes (\texttt{--nprocs}).
This executable must be explicitly compiled by
\begin{verbatim}
make mesalyzer
\end{verbatim}
with the resultant executable called \texttt{mesalyzer}. It cannot be used to display a window on the screen. 
The executable still expects an available Xserver but does not actually use it. 
Thus, you need to forward X connections if 
remotely connecting using \texttt{ssh}.
If running remotely, you probably also need to set the following environment variable:
\begin{verbatim}
export LIBGL_ALWAYS_INDIRECT=y
\end{verbatim}
If you are running on a headless system
(or having display troubles), you can try running a virtual X server with the command \texttt{Xvfb}.
Below is an example with a bash shell.
\begin{verbatim}
Xvfb :1 &
export DISPLAY=:1
mesalyzer ....

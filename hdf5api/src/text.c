...
int ch5_text_create(hid_t hdf_file, const char *label) {
  ...
  hid_t text_id = ch5_gnrc_open_or_create_chunked_dset(container_id,
    file_name, H5T_NATIVE_CHAR, 1, (hsize_t[1]){ 0 }, (hsize_t[1]){ H5S_UNLIMITED },
...
int ch5_text_append(hid_t hdf_file, unsigned int index, const char *in) {
  ...
  status = H5Dset_extent(text_id, (hsize_t[1]){ dims[0] + len });
  if (status < 0) return 1;
  space_id = H5Dget_space(text_id);/* (important!) */
  
  /* Select new range in hyperslab and write */
  status = H5Sselect_hyperslab(space_id, H5S_SELECT_SET,
    (hsize_t[1]){ dims[0] }, NULL, (hsize_t[1]){ len }, NULL);
  if (status < 0) {
    H5Sclose(space_id);
    H5Dclose(text_id);
    return 1;
  }
  hid_t memspace_id = H5Screate_simple(1, (hsize_t[1]){ len }, NULL);
  
  status = H5Dwrite(text_id, H5T_NATIVE_CHAR, memspace_id, space_id,
    H5P_DEFAULT, in);
...
* 
* When reading back, ensure that the amount of memory allocated for \p out is:
*
* <tt>sizeof(char) * (info.size + 1)</tt>
*
* Where <tt>info.size</tt> is the size reported from a #ch5_text_file struct.
*
* \note This function will take care of ensuring the string is null terminated
*       as long as there is sufficient memory allocated.
*
* \param[in]  hdf_file The HDF file reference id
* \param[in]  index    The index of the file to read from
* \param[out] out      A pointer to a string with sufficient space (see
...
int ch5_text_read(hid_t hdf_file, unsigned int index, char *out) {
  ...
  status = H5Dread(text_id, H5T_NATIVE_CHAR, H5S_ALL, H5S_ALL, H5P_DEFAULT, out);
  H5Dclose(text_id);
  if (status < 0) return 1;

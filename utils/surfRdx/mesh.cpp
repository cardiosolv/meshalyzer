...
// carpf = treu for CARP format 
Mesh::Mesh(char* filename, bool carpf)
{
    ...
    if( !carpf ) 
      ...
      success = loadFromCARPFile(filename);
    if( !success )
      {
...
void
Mesh :: loadTri( int v1, int v2, int v3, int i, int s )
{
...
bool
Mesh::loadFromCARPFile(string filename)
{
  char line[1024];

  //read in point file
  string ptf = filename+".pts";
  FILE *ptin = fopen( ptf.c_str(), "r" );
  fgets( line, 1024, ptin );
  sscanf( line, "%d", &_numVerts );
  ...
  string elemf = filename+".surf";
  FILE *elin   = fopen( elemf.c_str(), "r" );
  int   surfno = -1;
  while( !feof(elin) ) {
    fgets( line, 1024, elin );
    int numElem;
    char surfname[2048] = "";
    if( !sscanf( line, "%d %s", &numElem, &surfname ) )
      break;
    surfno++;
    if( !strlen(surfname) )
      sprintf( surfname, "surf%d", surfno );
    _surfnames.push_back(surfname);
    for( int i=0; i<numElem; i++ ) {
      fgets( line, 1024, elin );
      int v1, v2, v3, v4;
      char etype[3];
      if( sscanf( line, "%s %d %d %d %d", etype, &v1, &v2, &v3, &v4 )>=4 ) {
        loadTri( v1, v2, v3, _numTriangles++, surfno );
        if( !strcmp(etype, "Qd" ) )
          loadTri( v3, v4, v1, _numTriangles++, surfno );
      } else {
        MessageBox(NULL, "Reached End of File before all faces found!\n",
                NULL, MB_ICONEXCLAMATION);
        return false;
